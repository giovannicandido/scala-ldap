...
/**
  ...
  * TODO attributes are better as a Seq[LdapAttribute] where Seq.empty is None
  *
...
case class LdapEntry(dn: DN, attributes: Option[Seq[LdapAttribute]]) {
  ...
  def hasAttributeWithValue(attributeName: String, value: String): Boolean = {
    attributes.exists(_.exists(a => a.name.equalsIgnoreCase(attributeName) && a.value.contains(value)))
  }
  ...
  /**
    * Return if the objectClass attribute has the class name value
    * @param className Class name to compare
    * @return True or false
    */
  def hasObjectClass(className: String): Boolean = {
    // TODO performance could be improved
    attributes.exists(_.exists(a => {
      a.name.equalsIgnoreCase("objectClass") &&
        a.value.exists(v => {
          val slice = v.split("\\|")
          slice.exists(_.equalsIgnoreCase(className))
        })
    }
    ))
  }
