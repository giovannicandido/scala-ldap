...
object SearchScope extends Enumeration {
  ...
  /**
   * A predefined <b>baseObject</b> scope value, which indicates that only the entry specified by the base DN
   * should be considered.
   */
  val BASE = Value
  /**
   * A predefined <b>singleLevel</b> scope value, which indicates that only entries that are immediate subordinates of
   * the entry specified by the base DN (but not the base entry itself) should be considered.
   */
  val ONE = Value
  /**
   * A predefined <b>wholeSubtree</b> scope value, which indicates that the base entry itself and any
   * subordinate entries (to any depth) should be considered.
   */
  val SUB = Value
  /**
   * A predefined <b>subordinateSubtree</b> scope value, which indicates that any subordinate entries (to any depth) below
